...
export class MoodJournalSettingTab extends PluginSettingTab {
  ...
  override display(): void {
    const { containerEl } = this; const locale = this.plugin.moodSettings.locale; containerEl.empty(); containerEl.createEl('h2', { text: t(locale, 'settings.title') });
    new Setting(containerEl).setName(t(locale, 'settings.language')).addDropdown((drop) => drop.addOption('ja', '日本語').addOption('en', 'English').setValue(locale).onChange(async (value) => { if (value === 'ja' || value === 'en') { this.plugin.moodSettings.locale = value; await this.plugin.saveSettings(); this.display(); new Notice(t(value, 'notice.commandNameReload')); } }));
    this.heading(containerEl, t(locale, 'settings.daily'), t(locale, 'settings.dailyHelp'));
    new Setting(containerEl).setName(t(locale, 'settings.destination')).setDesc(t(locale, 'settings.destinationHelp')).addDropdown((drop) => drop.addOption('follow-core', t(locale, 'setup.follow')).addOption('manual', t(locale, 'setup.manual')).setValue(this.plugin.moodSettings.dailyNote.mode).onChange(async (value) => { if (value === 'follow-core' || value === 'manual') { this.plugin.moodSettings.dailyNote.mode = value; await this.plugin.saveSettings(); this.display(); } }));
  ...
  private heading(container: HTMLElement, title: string, helpText: string): HTMLElement { const row = container.createDiv({ cls: 'mood-journal-settings-heading' }); row.createEl('h3', { text: title }); const help = row.createEl('button', { text: '?', cls: 'mood-journal-help-button', attr: { 'aria-label': `${title}の説明`, title: helpText, 'aria-expanded': 'false' } }); const description = container.createDiv({ text: helpText, cls: 'mood-journal-heading-help mood-journal-hidden' }); help.onclick = () => { const hidden = description.hasClass('mood-journal-hidden'); description.toggleClass('mood-journal-hidden', !hidden); help.setAttribute('aria-expanded', String(hidden)); }; return row; }
  ...
  private async saveTextValue(value: string, update: (value: string) => Promise<void>): Promise<void> { try { await update(value); this.plugin.scheduleSettingsSave(); } catch { new Notice(t(this.plugin.moodSettings.locale, 'settings.invalidValue')); this.display(); } }
  private tagSetting(container: HTMLElement, id: string, label: string, hidden: boolean, tag: string, child: boolean): void { const locale = this.plugin.moodSettings.locale; let isHidden = hidden; const setting = new Setting(container).setName(child ? `↳ ${label}` : label).setDesc(isHidden ? t(locale, 'settings.hidden') : tag).addText((text) => { text.setValue(label); text.inputEl.addEventListener('blur', () => void this.renameTag(id, label, text.inputEl.value)); }).addButton((button) => button.setButtonText(isHidden ? t(locale, 'settings.restore') : t(locale, 'settings.hide')).onClick(async () => { isHidden = !isHidden; this.plugin.moodSettings.activities = new ActivityService().setHidden(this.plugin.moodSettings.activities, id, isHidden, new Date().toISOString()); await this.plugin.saveSettings(); setting.setDesc(isHidden ? t(locale, 'settings.hidden') : tag); button.setButtonText(isHidden ? t(locale, 'settings.restore') : t(locale, 'settings.hide')); setting.settingEl.toggleClass('mood-journal-tag-is-hidden', isHidden); })); setting.settingEl.toggleClass('mood-journal-tag-is-hidden', isHidden); if (child) setting.settingEl.addClass('mood-journal-tag-child'); }
  private async renameTag(id: string, previous: string, value: string): Promise<void> { if (value === previous) return; try { this.plugin.moodSettings.activities = new ActivityService().rename(this.plugin.moodSettings.activities, id, value, new Date().toISOString()); await this.plugin.saveSettings(); this.display(); } catch { new Notice(t(this.plugin.moodSettings.locale, 'settings.tagRenameError')); this.display(); } }
}
