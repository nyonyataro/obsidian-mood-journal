...
export class JournalEntryModal extends Modal {
  ...
  private render(): void {
    ...
    for (const score of [5, 4, 3, 2, 1] as MoodScore[]) {
      ...
      button.setAttribute('aria-pressed', String(this.draft.moodScore === score));
      button.onclick = () => {
        this.draft.moodScore = this.draft.moodScore === score ? null : score;
        this.render();
      };
    }
    ...
    const memo = body.createEl('textarea', {
      cls: 'mood-journal-memo',
      attr: {
        rows: '3',
        placeholder: t(locale, 'entry.memo'),
        'aria-label': t(locale, 'entry.memo'),
      },
    });
    memo.value = this.draft.memo;
    memo.oninput = () => {
      this.draft.memo = memo.value;
    };

    const change = body.createEl('button', {
      text: this.draft.dateTimeMode === 'manual' ? t(locale, 'entry.closeDate') : t(locale, 'entry.changeDate'),
    });
    ...
    change.onclick = () => {
      ...
      if (this.draft.dateTimeMode === 'manual') {
        ...
        this.draft.manualDate ||= localDateInputValue(now);
        this.draft.manualTime ||= localTimeInputValue(now);
      }
      this.render();
    };

    if (this.draft.dateTimeMode === 'manual') {
      const inputs = body.createDiv({ cls: 'mood-journal-date-inputs' });
      const date = inputs.createEl('input', {
        attr: {
          type: 'date',
          value: this.draft.manualDate,
          'aria-label': t(locale, 'entry.date'),
        },
      });
      date.onchange = () => {
        this.draft.manualDate = date.value;
      };
      const time = inputs.createEl('input', {
        attr: {
          type: 'time',
          value: this.draft.manualTime,
    ...
    if (this.error) {
      const error = body.createDiv({ cls: 'mood-journal-error', attr: { role: 'alert' } });
    ...
    const save = footer.createEl('button', {
      text: this.saving ? t(locale, 'entry.saving') : t(locale, 'entry.save'),
      cls: 'mod-cta',
    });
    save.disabled = this.draft.moodScore === null || this.saving;
  ...
  private renderTags(contentEl: HTMLElement, locale: 'ja' | 'en'): void {
    ...
    header.createEl('button', { text: `+ ${t(locale, 'entry.addActivity')}` }).onclick = () =>
      new ActivityEditorModal(this.plugin, (id) => {
        ...
        if (tag !== undefined) {
          this.draft.activityIds = toggleActivitySelection(
            this.draft.activityIds,
            tag,
            this.plugin.moodSettings.activities,
          );
        }
        this.render();
      }).open();
    ...
    const search = contentEl.createEl('input', {
      cls: 'mood-journal-activity-search',
      attr: {
        type: 'search',
        placeholder: t(locale, 'entry.searchTags'),
        'aria-label': t(locale, 'entry.searchTags'),
      },
    });
    search.value = this.query;
    const list = contentEl.createDiv({ cls: 'mood-journal-tag-picker' });
    const all = this.plugin.moodSettings.activities;
    const compare = (left: ActivityDefinition, right: ActivityDefinition): number =>
      left.sortOrder - right.sortOrder || left.label.localeCompare(right.label);
    const parents = all
      .filter((item) => item.parentId === null && !isEffectivelyHidden(item, all))
      .sort(compare);
    for (const parent of parents) {
      this.parentTagGroup(
        list,
        parent,
        all.filter((item) => item.parentId === parent.id && !isEffectivelyHidden(item, all)).sort(compare),
      );
    }
    const filter = (): void => {
      const query = search.value.trim().toLocaleLowerCase();
      for (const tree of Array.from(list.querySelectorAll<HTMLElement>('.mood-journal-tag-tree'))) {
        tree.toggleClass('mood-journal-hidden', Boolean(query) && !(tree.dataset.searchText ?? '').includes(query));
      }
    };
    search.oninput = () => {
  ...
  private parentTagGroup(container: HTMLElement, parent: ActivityDefinition, children: ActivityDefinition[]): void {
    const tree = container.createDiv({ cls: 'mood-journal-tag-tree' });
    tree.dataset.searchText = `${parent.label} ${parent.slug} ${children
      .map((child) => `${child.label} ${child.slug}`)
      .join(' ')}`.toLocaleLowerCase();
    const row = tree.createDiv({ cls: 'mood-journal-tag-parent-row' });
    this.tagButton(row, parent, 'mood-journal-tag-parent');
    if (children.length === 0) return;
    const childList = tree.createDiv({ cls: 'mood-journal-tag-children' });
    childList.createDiv({
      text: '↳',
      cls: 'mood-journal-tag-child-marker',
      attr: { 'aria-hidden': 'true' },
    });
    for (const child of children) {
      this.tagButton(childList, child, 'mood-journal-tag-child-button');
    }
  }

  private tagButton(container: HTMLElement, tag: ActivityDefinition, cls: string): void {
    const button = container.createEl('button', {
      text: tag.label,
      cls: `mood-journal-activity ${cls}`,
    });
    button.setAttribute('aria-pressed', String(this.draft.activityIds.includes(tag.id)));
    button.onclick = () => {
      this.draft.activityIds = toggleActivitySelection(
        this.draft.activityIds,
        tag,
  ...
  private isDirty(): boolean {
    return isDraftDirty(this.draft);
  }

  private async save(): Promise<void> {
    if (this.saving || this.draft.moodScore === null) return;
    this.saving = true;
    this.error = '';
    this.render();
    try {
      ...
      console.error(
        '[mood-journal]',
        code,
        cause instanceof MoodJournalError ? cause.safeContext : {},
        cause,
      );
    } finally {
      this.saving = false;
