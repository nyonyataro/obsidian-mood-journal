...
export interface ActivityDefinition { id: string; parentId: string | null; label: string; slug: string; hidden: boolean; sortOrder: number; usageCount: number; lastUsedAt: string | null; createdAt: string; updatedAt: string; }
...
export interface ActivitySnapshot { activityId: string; labelPath: string; tag: string; }
export interface JournalEntry { id: string; occurredAt: string; moodScore: MoodScore; moodEmoji: string; moodLabel: string; activities: ActivitySnapshot[]; memo: string; }
